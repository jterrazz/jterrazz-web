...
Ever stop and think about how your computer is actually shuffling trillions of bytes around every second? 🤹‍♂️ It's a question that always fascinated me. So, I decided to pull back the curtain and dive into one of the most fundamental pieces of the puzzle: **dynamic memory allocation**.

In this post, I'll walk you through why `malloc` even exists, how it works on a deep level, and how I built my own version from scratch using the `mmap` system call. If that sounds complex, don't worry. I'm going to break it down from first principles. For me, understanding this stuff was a real "aha!" moment. And if you want to get your hands dirty, my [completed project is on GitHub](https://github.com/jterrazz/42-malloc). Let's get into it. 🚀
...
2. **You're stuck with a fixed lifespan.** The memory either lasts forever or for the duration of one function call. Nothing in between.
...
These little macros were helpers to quickly jump from the start of a heap/block to the user-data area.
...
A quick pro-tip I learned: it's way more efficient to make your heap sizes a multiple of the system's page size. You can get this with `getpagesize()` (or `getconf PAGE_SIZE` in the terminal). On my machine, it's 4096 bytes.
...
`realloc` is really just a simple recipe: `malloc` a new, larger block, `memcpy` the data from the old block to the new one, and `free` the old block.

One edge case to be aware of: `realloc(ptr, 0)`. The behavior here can vary. I took a "lazy" approach and just returned the original pointer. However, you should know that some standards say this should be equivalent to `free(ptr)`. My advice: don't use `realloc` to `free` memory. Use the right tool for the job.
...
And that's the journey! We went from the kernel's `mmap` call all the way to a functioning, tested `malloc` library. For me, this project wasn't just about code; it was about demystifying a core part of how our machines operate.

Practice is everything. If you want to dig deeper, please check out the [complete implementation on my GitHub](https://github.com/jterrazz/42-malloc). Fork it, break it, make it better. Understanding the foundation gives you the power to build anything. Happy coding
