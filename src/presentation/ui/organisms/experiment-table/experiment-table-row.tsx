...
import { ExperimentTableRowComponent } from "./experiment-table-row-component";
import { experimentStatusToBadgeColor } from "./experiment-table-row-view-model";

export type ExperimentTableRowProps = {
  className?: string;
  experiment: Experiment;
};

export const ExperimentTableRow: React.FC<ExperimentTableRowProps> = ({
  className,
  experiment,
}) => {
          ...
          <div className="lg:sticky lg:top-32 space-y-6">
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                <Badge
                  color={experimentStatusToBadgeColor(experiment.status)}
          ...
          {activeComponents.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {activeComponents.map((component) => (
                <ExperimentTableRowComponent component={component} key={component.name} />
              ))}
            </div>
          ) : (
            <div className="p-8 rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-800 text-center">
              <p className="text-zinc-500 dark:text-zinc-500">No active components visible.</p>
            </div>
          )}
        </div>
      </div>
