...
function LanguageSwitcher({ className, onSwitch }: { className?: string; onSwitch?: () => void }) {
  ...
  return (
    <div className={cn("relative", className)} ref={dropdownRef}>
      ...
      >
        {locale.toUpperCase()}
        <IconChevronDown className={cn("transition-transform", isOpen && "rotate-180")} size={12} />
      </button>
...
export const Navbar: React.FC<NavbarProps> = ({ className, contacts, pages, translations: t }) => {
  ...
  return (
    <>
      <nav className={cn("w-full flex justify-center p-4 md:pt-6 pointer-events-none", className)}>
        <div
          className={cn(
              ...
              <div className="flex items-center gap-6">
                {contacts.map((contact) => (
                  <a
                    ...
                    target="_blank"
                  >
                    {getContactIcon(contact.name) || <IconDeviceDesktop size={20} />}
                  </a>
                ))}
